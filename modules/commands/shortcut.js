...
module.exports.onLoad = function () {
    ...
    const pathGif = resolve(__dirname, '..', 'commands' ,"cache", "shortcut");
...
module.exports.handleEvent = async function ({ event, api, Users }) {
    ...
    if (data.some(item => item.input == body)) {
        ...
        const path = resolve(__dirname, '..', 'commands' ,"cache", "shortcut",`${dataThread.id}`);
        ...
        case "requireGif": {
            ...
            if ((event.attachments||[]).length > 0) {
              ...
              id=id+'.'+format_attachment(atm_0.type);
                const pathGif = resolve(__dirname, '..', 'commands' ,"cache", "shortcut", id);
        ...
        case "-a": {
            ...
            if (data.length == 0) return api.sendMessage("[ Shortcut ] - hiện tại nhóm của bạn chưa có shortcut nào được set!", threadID, messageID);
                ...
                for (const single of data) {
                    const path = resolve(__dirname, '..', 'commands' ,"cache", "shortcut",`${single.id}`);
                    var existPath = false;
