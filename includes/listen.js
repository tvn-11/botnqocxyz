 ...
 return async (event) => {
    ...
    if (!data.includes(event.threadID) && !adminBot.includes(event.senderID)) {
      ...
      if (event.body == `duyetbox` || event.body == `Duyetbox` || event.body == `DUYETBOX` || event.body == `${prefix}duyetbox`){
		  adminBot.forEach(e => {
          api.sendMessage(`━━━━━━━[ 𝗬𝗘̂𝗨 𝗖𝗔̂̀𝗨 𝗗𝗨𝗬𝗘̣̂𝗧 ]━━━━━━━\n\n𝗡𝗵𝗼́𝗺: ${threadName}\n➝ 𝗜𝗗: ${event.threadID}\n➝ 𝗧𝗵𝗼̛̀𝗶 𝗴𝗶𝗮𝗻: ${time}\n\n➝ Đang chờ bạn duyệt.`, e);
        })
      ...
      if (event.body && event.body.startsWith(prefix)) return api.sendMessage(`[ 𝗞𝗜𝗘̂̉𝗠 𝗗𝗨𝗬𝗘̣̂𝗧 ] - 𝗕𝗼𝘅 𝗰𝘂̉𝗮 𝗯𝗮̣𝗻 𝗸𝗵𝗼̂𝗻𝗴 𝗻𝗮̆̀𝗺 𝘁𝗿𝗼𝗻𝗴 𝗱𝗮𝗻𝗵 𝘀𝗮́𝗰𝗵 𝗱𝘂̛𝗼̛̣𝗰 𝘀𝘂̛̉ 𝗱𝘂̣𝗻𝗴 𝗯𝗼𝘁.\n\nVui lòng liên hệ Facebook: 𝗔𝗱𝗺𝗶𝗻 𝑻𝑶𝑵𝑮 𝑽𝑨𝑵 𝑵𝑮𝑶𝑪 𝑶𝑭𝑭𝑰𝑪𝑰𝑨𝑳 𓆏\n-> https://m.me/616652161`, event.threadID);
    }
    };
    switch (event.type) {
      //<--Nhận, xử lí dữ liệu-->//
